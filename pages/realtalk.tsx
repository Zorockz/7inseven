...
const RealTalk = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // API call - use your backend endpoint
      ...
      <div className="w-full max-w-md">
        <Card className="bg-white/90 backdrop-blur-sm p-6 rounded-3xl shadow-2xl">
          <div className="text-center mb-6">
            ...
            <h1 className="text-2xl font-bold text-gray-800 mb-1">RealTalk GPT</h1>
                  ...
                  {tones.map((tone) => {
                    ...
                    return (
                      <div key={tone.id} className="relative">
                        ...
                        >
                          <div className={`p-2 rounded-full ${tone.color} mb-2`}>
                            <Icon className="w-4 h-4 text-white" />
                          </div>
                          <span className="font-medium text-sm text-gray-800">{tone.name}</span>
          ...
          {response && (
            <div className="mt-6 p-4 bg-gradient-to-r from-gray-50 to-gray-100 rounded-2xl">
              <div className="flex items-center mb-2">
                {selectedToneData && (
                  <div className={`p-2 rounded-full ${selectedToneData.color} mr-3`}>
                    <selectedToneData.icon className="w-4 h-4 text-white" />
                  </div>
                )}
                <span className="font-medium text-gray-800 capitalize">{selectedTone} Advice</span>
              </div>
